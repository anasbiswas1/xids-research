...
**Document version:** v7 (model-ranking correction)
...
**Headline:** XGBoost is the best model on 5-class macro-F1 across all three datasets. The RF vs DNN ordering is dataset-dependent: NSL-KDD shows XGB > DNN (0.584) > RF (0.525); CIC-IDS2017 and UNSW-NB15 show XGB > RF > DNN.
...
| 6 | XGBoost is the best model on 5-class macro-F1 across all three datasets | NSL-KDD 0.639, CIC-IDS2017 0.980, UNSW-NB15 0.582. RF vs DNN ordering is dataset-dependent (DNN > RF on NSL; RF > DNN on CIC and UNSW) | NSL, CIC, UNSW | ✅ |
...
### v7 (model-ranking fix, performed 27 May 2026)

A third pass identified that the "XGB > RF > DNN ranking holds across datasets" claim is **factually wrong**. Checking the actual macro-F1 numbers in Table 1:

| Dataset | RF | XGB | DNN | Actual ranking |
|---|---:|---:|---:|---|
| NSL-KDD | 0.525 | 0.639 | 0.584 | XGB > DNN > RF |
| CIC-IDS2017 | 0.975 | 0.980 | 0.792 | XGB > RF > DNN |
| UNSW-NB15 | 0.526 | 0.582 | 0.517 | XGB > RF > DNN |

XGBoost is the top model on all three datasets, but the RF vs DNN ordering is dataset-dependent: DNN beats RF on NSL-KDD, RF beats DNN on the other two datasets.

**Corrections applied in v7:**
- Table 1 headline rewritten: "XGBoost is the best model on 5-class macro-F1 across all three datasets" with explicit RF-vs-DNN ordering by dataset
- Table 8 Claim #6 rewritten with the same precision

**Carry-forward correction in walkthrough v5:** The same error appeared in three places in walkthrough v3/v4 (§5.3 baseline finding, §13.2 cross-dataset findings, §15.1 headline findings). All three corrected in walkthrough v5.
...
- v6: Second audit. Fixed NSL stability sample size, Table 6 ranking claim, Table 8 Claim #11 sample-size attribution, Table 4 bolding consistency.
- **v7 (current):** Third audit. Fixed model-ranking claim (XGB best on all three, but RF vs DNN ordering is dataset-dependent — DNN > RF on NSL only). Carry-forward fix applied in walkthrough v5.
