...
**Notable:** Two unexpected NSL improvements appeared in v2:
- *DNN binary (+2.1 pp):* Despite 20% less training data. Most likely single-seed variance; not to be claimed as a v2 methodology benefit.
- *RF binary (+1.2 pp) and RF 5-class SMOTE (~0 acc but +0.6 pp F1m):* These came from applying the max_depth=20 cap in a retrain pass. Constraining depth reduced overfitting on NSL's small training set. This is a genuine side-finding worth noting in the paper: the M10 cap, originally motivated by TreeExplainer feasibility, also improves RF generalisation on NSL.
...
**RF max_depth verification (UNSW):** rf_binary_cw, rf_5class_smote, rf_5class_cw all hit `mean=20.0, max=20`. The depth=20 cap was active on every tree. Without the cap, trees would have grown to v1's depth-46+ range. **M10 SHAP-infeasibility issue permanently resolved for UNSW.**
...
**RF max_depth verification (CIC):** rf_binary_cw hit `mean=20.0, max=20` — same as UNSW. CIC's RF would also have grown unconstrained without the cap, even though v1 didn't visibly suffer from it. The cap is now correctly applied to all three datasets.
...
6. **Unexpected positive side-finding:** the depth cap improved NSL RF performance (rf_binary_cw +1.2pp acc, rf_5class_smote +0.6pp F1m) through reduced overfitting on the small NSL training set. The cap was originally motivated by TreeExplainer feasibility; the generalisation benefit is a bonus worth documenting.
